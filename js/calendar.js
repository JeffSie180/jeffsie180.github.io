...
(function ($) {
  var aCalendar = function (language, options, object) {
    ...
    var settings = $.extend(
      ...
      typeof calLanguages === "undefined" ? {} : calLanguages[currentLanguage],
      options,
    );
    ...
    function parse() {
      ...
      if (
        ...
        ((current.prev === null &&
          current.next !== null &&
          current.next.getTime() > time) ||
          (current.next === null &&
            current.prev !== null &&
            current.prev.getTime() < time))
      ) {
        return false;
      }
    ...
    function simpleDateFormat(date, fmt) {
      ...
      for (var k in o) {
        if (new RegExp("(" + k + ")").test(fmt)) {
          fmt = fmt.replace(
            RegExp.$1,
            k === "LMM+" ? o[k] : ("00" + o[k]).substr(("" + o[k]).length),
          );
        }
      }
    ...
    function draw() {
      ...
      var dWeekDayOfMonthStart =
        new Date(dYear, dMonth, 1).getDay() - settings.weekOffset;
      ...
      var dLastDayOfPreviousMonth =
        new Date(dYear, dMonth, 0).getDate() - dWeekDayOfMonthStart + 1;
      ...
      if (current.posts.length === 0) {
        ...
        cTitleLink = $("<a/>")
          .attr("href", simpleDateFormat(curDate, settings.titleLinkFormat))
      ...
      for (var i = 0; i < 7; i++) {
        ...
        var cWeekDay = $("<th/>")
          .attr("scope", "col")
          .attr("title", settings.dayOfWeek[dayOfWeek]);
      ...
      if (current.prev) {
        cPrevPosts
          .html(
            settings.footArrows.previous +
              settings.months[current.prev.getMonth()],
          )
          .addClass("cal-foot")
          .attr(
            "title",
            simpleDateFormat(current.prev, settings.postsMonthTip),
          );
      }
      ...
      if (current.next) {
        cNextPosts
          .html(
            settings.months[current.next.getMonth()] + settings.footArrows.next,
          )
          .addClass("cal-foot")
          .attr(
            "title",
            simpleDateFormat(current.next, settings.postsMonthTip),
          );
      }
        ...
        for (var j = 0; j < 7; j++) {
          ...
          if (i * 7 + j < dWeekDayOfMonthStart) {
            ...
            if (count.num !== 0) {
              ...
              var cLink = $("<a>")
                .attr("href", current.posts[index].link)
                .attr("title", current.posts[index].title)
                .html(day++);
              cDay.append(cLink);
            } else {
              cDay.html(day++);
